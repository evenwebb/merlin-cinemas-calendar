...
Scrapes upcoming film releases from Merlin Cinemas and generates iCalendar files.
"""
import datetime
import hashlib
import json
...
CHAIN_NAME = "Merlin Cinemas"
CHAIN_SHORT_NAME = "Merlin"
ICAL_DOMAIN = "merlin-cinemas-calendar"
ICAL_FILE_PREFIX = "merlin"
SOURCE_WEBSITE_URL = "https://www.merlincinemas.co.uk/"
SOURCE_REPO_URL = "https://github.com/steven/merlin-cinemas-calendar"
...
SYNOPSIS_SKIP_TERMS = [
    'cookie', 'privacy', 'terms', 'wheelchair', 'audio description',
    'merlin cinemas', 'movie magic', 'change location', 'cinema info',
    'capitol cinema', 'flora cinema', 'phoenix cinema', 'royal cinema',
    'savoy cinema', 'regal cinema', 'the ritz', 'cornwall',
]
...
DATE_PATTERN = re.compile(r"Expected:\s*(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})", re.IGNORECASE)
# Alternative pattern for "Expected at <cinema> from the DDth Month" style strings
ALT_DATE_PATTERN = re.compile(
    r"Expected(?:\s+at\s+[A-Za-z\s&.']+)?(?:\s+from(?:\s+the)?)?\s+(\d{1,2})(?:st|nd|rd|th)?\s+([A-Za-z]+)(?:\s+(\d{4}))?",
    re.IGNORECASE,
)
...
def get_base_film_url(url: str) -> str:
    ...
    Args:
        url: Full URL like 'https://bodmin.merlincinemas.co.uk/film/105263-wuthering-heights'

    Returns:
        Base URL used as cache key (query removed and Merlin film URLs normalised)
    """
    if '?' in url:
        url = url.split('?')[0]

    parsed = urlparse(url)
    if parsed.netloc.endswith("merlincinemas.co.uk") and "/film/" in parsed.path:
        # Share cache across cinema subdomains for the same film page.
        return f"merlin:{parsed.path.rstrip('/').lower()}"
    return url
...
def save_cache(cache: Dict[str, dict]) -> None:
    ...
    """
    try:
        serialised = json.dumps(cache, indent=2, ensure_ascii=False)
        if os.path.exists(CACHE_FILE):
            existing = Path(CACHE_FILE).read_text(encoding='utf-8')
            if existing == serialised:
                logger.info("Cache unchanged (%d entries)", len(cache))
                return
        Path(CACHE_FILE).write_text(serialised, encoding='utf-8')
...
def save_release_history(releases: set) -> None:
    ...
    kept = sorted((d.isoformat(), t) for (d, t) in releases if d >= cutoff)
    try:
        serialised = json.dumps(kept, ensure_ascii=False)
        if path.exists() and path.read_text(encoding="utf-8") == serialised:
            logger.info("Release history unchanged (%d entries)", len(kept))
            return
...
def save_tmdb_cache(cache: Dict[str, dict]) -> None:
    ...
    try:
        ...
        if os.path.exists(TMDB_CACHE_FILE):
            existing = Path(TMDB_CACHE_FILE).read_text(encoding='utf-8')
            if existing == serialised:
                logger.info("TMDb cache unchanged (%d entries)", len(cache))
                return
        Path(TMDB_CACHE_FILE).write_text(serialised, encoding='utf-8')
    except OSError as e:
        logger.warning("TMDb cache save failed: %s", e)


def write_text_if_changed(path: Path, content: str) -> bool:
    """Write text file only when content changes.

    Returns:
        True if file content changed and was written, otherwise False.
    """
    try:
        if path.exists():
            # Preserve newlines exactly (important for .ics files that require CRLF).
...
def parse_date(text: str) -> Optional[datetime.date]:
    ...
    2. "Expected at <cinema> from the DDth Month" (e.g., "Expected at Merlin Cinemas from the 10th October")
    3. Generic day headings like "Saturday 28th February" or "We're expecting this on Friday 6th March."
    ...
    for month_fmt in ("%B", "%b"):
        try:
            month = datetime.datetime.strptime(month_str, month_fmt).month
            break
        except ValueError:
            continue
    if month is None:
    ...
    try:
        ...
        # If no explicit year and this appears to be a future listing, roll over into next year.
        text_lower = text.lower()
        has_explicit_year = bool(re.search(r"\b\d{4}\b", text))
        is_future_context = any(token in text_lower for token in ("expected", "showing", "coming"))
        if (
            parsed_date < datetime.date.today()
            and year == datetime.date.today().year
            and not has_explicit_year
            and is_future_context
        ):
...
def fetch_film_details(film_url: str, cache: Dict[str, dict]) -> Dict[str, str]:
    ...
    try:
        ...
        # Guard against Merlin location-navigation text being mistaken for synopsis.
        synopsis = details['synopsis']
        if synopsis:
            nav_markers = (
                "Capitol Cinema", "Flora Cinema", "Phoenix Cinema", "Regal Cinema",
                "Royal Cinema", "Savoy Cinema", "The Ritz", "Embassy Cinema",
                "New Central Cinema", "Tivoli Cinema",
            )
            if synopsis.count("Cinema") >= 2 or any(marker in synopsis for marker in nav_markers):
                details['synopsis'] = ''
...
def extract_films(
    ...
    # Merlin coming-soon cards: release date is shown in div.release_date inside each film card.
    release_cards = soup.select("div.filmCard[data-film]")
    if any(card.find("div", class_="release_date") for card in release_cards):
        for card in release_cards:
            slug = (card.get("data-film") or "").strip()
            if not slug:
                continue
            if "internal_advert" in (card.get("class") or []):
                continue

            title_elem = card.find("h2", class_=re.compile(r"\boverlay\b")) or card.find("h2")
            release_elem = card.find("div", class_="release_date")
            if not title_elem or not release_elem:
                continue

            title = re.sub(r"\s*\([^)]*\)$", "", title_elem.get_text(strip=True))
            release_text = release_elem.get_text(" ", strip=True)
            release_date = parse_date(release_text)
            if not release_date or release_date < today:
                continue

            film_url = urljoin(url, f"/film/{slug}")
        ...
        films.sort(key=lambda x: (x[0], x[1]))
        return films

    # Merlin format: each date frame has data-frame="YYYY-MM-DD" with nested film cards.
    # This gives stable release dates per film without scraping free-form "Expected..." text.
    frame_nodes = soup.select("div.films_on_day > div[data-frame]")
    dated_frames = [f for f in frame_nodes if (f.get("data-frame") or "").strip().lower() != "full"]

    if dated_frames:
        release_by_slug: Dict[str, datetime.date] = {}
        title_by_slug: Dict[str, str] = {}

        for frame in dated_frames:
            frame_date_raw = (frame.get("data-frame") or "").strip()
            try:
                frame_date = datetime.datetime.strptime(frame_date_raw, "%Y-%m-%d").date()
            except ValueError:
                continue

            for card in frame.find_all("div", class_=re.compile(r"\bfilmCard\b")):
                slug = (card.get("data-film") or "").strip()
                if not slug:
                    continue
                if "internal_advert" in (card.get("class") or []):
                ...
                if slug not in release_by_slug or frame_date < release_by_slug[slug]:
                    release_by_slug[slug] = frame_date
                    title_by_slug[slug] = title

        # Fallback: if a film appears only in "full" view with no dated frame, parse <h5> headings.
        full_frame = soup.find("div", attrs={"data-frame": "full"})
        if full_frame:
            for card in full_frame.find_all("div", class_=re.compile(r"\bfilmCard\b")):
                slug = (card.get("data-film") or "").strip()
                if not slug or slug in release_by_slug:
                    continue
                if "internal_advert" in (card.get("class") or []):
                    continue
                ...
                for h5 in card.find_all("h5"):
                    d = parse_date(h5.get_text(" ", strip=True))
                    if d:
                        candidate_dates.append(d)
                if candidate_dates:
                    release_by_slug[slug] = min(candidate_dates)
                    title_by_slug[slug] = title

        for slug, release_date in sorted(release_by_slug.items(), key=lambda item: (item[1], title_by_slug.get(item[0], ""))):
            title = title_by_slug.get(slug, "").strip()
            if not title:
                continue
            film_url = urljoin(url, f"/film/{slug}")
            film_details = fetch_film_details(film_url, cache)
    ...
    # WTW-style fallback (legacy parser)
    ...
    for times_div in times_divs:
        ...
        if not date_elem:
            continue
        release_date = parse_date(date_elem.get_text(strip=True))
        film_url = ""
...
def _cast_first_six_names(cast_str: str) -> str:
    """From cast strings like 'Name1 (Character1), Name2 (Character2)' return first 6 names only."""
...
def make_ics_event(
    ...
    Layout A (cinema-only): title (runtime), description, starring, footer.
    Layout B (TMDb-enriched): title (runtime), star rating, genres, description, starring, footer.
    Runtime is always from cinema page. Cast shows first 6 names only (no character names).
    """
    dtend = release_date + datetime.timedelta(days=1)
    summary = f"{film_title} @ {CHAIN_SHORT_NAME} {cinema_name}"
    uid_seed = f"{release_date.isoformat()}|{film_title}|{cinema_name}|{film_url or ''}"
    uid = f"{hashlib.sha1(uid_seed.encode('utf-8')).hexdigest()}@{ICAL_DOMAIN}"
    # Stable DTSTAMP keeps unchanged feeds byte-identical across runs.
    dtstamp = datetime.datetime.combine(release_date, datetime.time(0, 0), tzinfo=datetime.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    details = film_details or {}

    # Runtime display (from cinema page)
    ...
    # Description paragraph (TMDb overview or scraped synopsis)
    ...
    # Starring: first 6 names only (strip character names from cast format)
    ...
    description_parts.append(f"\nFilm release at {CHAIN_NAME} {cinema_name}")
    if film_url:
    ...
    event_lines = [
        ...
        escape_and_fold_ical_text(f"{CHAIN_NAME} {cinema_name}", "LOCATION:"),
    ]
    if film_url:
...
def build_index_html(
    ...
    for cinema_id, info in enabled_cinemas.items():
        ...
        ics_url = f"{ICAL_FILE_PREFIX}-{cinema_id}.ics"
    ...
    # Keep a deterministic order for same-day releases to avoid no-change churn in index.html.
    featured = sorted(unique_films, key=lambda x: (x[0], x[1].lower()))[:6]
    featured_html = "".join(
...
<head>
  ...
  <title>{CHAIN_NAME} – Movie Premieres</title>
...
<body>
  ...
  <div class="page">
    <header class="hero">
      ...
      <h1>{CHAIN_NAME} Movie Premieres</h1>
      <p class="tagline">Subscribe to new movie premieres at your nearest {CHAIN_SHORT_NAME} cinema. Never miss a release date. Pick your cinema below and add the calendar to your phone or desktop.</p>
    </header>
...
{stats_html}
    ...
    <footer>
      <p class="footer-disclaimer">An open source fan-made project. Calendars update when new premieres are added. Not affiliated with {CHAIN_NAME}.</p>
      <div class="footer-links">
        <a href="{SOURCE_WEBSITE_URL}">{CHAIN_NAME}</a>
        <span aria-hidden="true">·</span>
        <a href="{SOURCE_REPO_URL}">Source</a>
        <span aria-hidden="true">·</span>
        <a href="{SOURCE_REPO_URL}">Project</a>
      </div>
    </footer>
  </div>
  <script>
...
def main() -> None:
    ...
    # Scrape enabled cinemas in parallel (network-bound workload).
    max_workers = min(8, len(enabled_cinemas))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {}
        for cinema_id, cinema_info in enabled_cinemas.items():
            cinema_name = cinema_info['name']
            cinema_url = cinema_info['url']
            future = executor.submit(extract_films, cinema_url, cinema_name, cache)
            future_map[future] = (cinema_id, cinema_name)

        for future in as_completed(future_map):
            cinema_id, cinema_name = future_map[future]
            try:
                films = future.result()
                for f in films:
    ...
    if api_key:
        ...
        logger.info("TMDB_API_KEY not set; using cinema-only data (Layout A)")
    ...
    # Sort films deterministically so generated files are byte-stable across runs.
    all_films.sort(key=lambda x: (x[0], x[2], x[1].lower(), x[3]))
    ...
    # Remove stale .ics files (legacy names or disabled/renamed cinemas)
    output_dir = Path(OUTPUT_DIR)
    expected_ics_names = {f"{ICAL_FILE_PREFIX}-{cinema_id}.ics" for cinema_id in enabled_cinemas}
    for old_ics in output_dir.glob("*.ics"):
        if old_ics.name not in expected_ics_names:
            old_ics.unlink()
    ...
    for cinema_id in enabled_cinemas:
        ...
        calendar_lines = [
            "BEGIN:VCALENDAR",
            "VERSION:2.0",
            f"PRODID:-//{CHAIN_NAME}//EN",
            "CALSCALE:GREGORIAN",
            "METHOD:PUBLISH",
            f"X-WR-CALNAME:{CHAIN_SHORT_NAME} {cinema_name} Movie Premieres",
            f"X-WR-CALDESC:Upcoming movie premieres at {CHAIN_NAME} {cinema_name}",
        ]
        ...
        out_path = Path(OUTPUT_DIR) / f"{ICAL_FILE_PREFIX}-{cinema_id}.ics"
        changed = write_text_if_changed(out_path, ical)
        logger.info("%s %s (%d events)", "Wrote" if changed else "Unchanged", out_path, len(events))
    ...
    index_changed = write_text_if_changed(index_path, index_html)
    logger.info("%s %s", "Wrote" if index_changed else "Unchanged", index_path)
